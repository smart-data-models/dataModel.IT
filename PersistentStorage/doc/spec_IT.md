...
[Licenza aperta](https://github.com/smart-data-models//dataModel.IT/blob/master/PersistentStorage/LICENSE.md)  
...
Global description: **Memoria permanente richiesta da un ServiceComponent.**  
...
- `accessMode[string]`: Modalità dell'archiviazione persistente richiesta. Enum: 'ReadWriteOnce', 'ReadOnlyMany', 'ReadWriteMany'  
...
- `hostPath[string]`: (Solo se storageType è 'local') Percorso host nell'Elemento di infrastruttura in cui è ubicato l'archiviazione persistente.  
...
- `mountPath[string]`: Percorso di montaggio nel ServiceComponent dell'archiviazione persistente richiesta.  
...
- `size[number]`: Dimensione dell'archiviazione persistente richiesta.  
...
- `storageType[string]`: Tipo di archiviazione richiesta, se locale è montata nella IE selezionata del ServiceComponent, altrimenti è montata in un'infrastruttura esterna come un sistema di archiviazione cloud. Enum: 'Esterno', 'Locale'  
- `type[string]`: Tipo di entità NGSI. Deve essere PersistentStorage  
<!-- /30-PropertiesList -->
...
#### Archiviazione Persistente chiavi-valori Esempio NGSI-v2  

Ecco un esempio di PersistentStorage in formato JSON come chiavi-valori. Ciò è compatibile con NGSI-v2 quando si utilizza `options=keyValues` e restituisce i dati di contesto di un'entità singola.  
...
#### Archiviazione persistente esempio normalizzato NGSI-v2  

Ecco un esempio di PersistentStorage in formato JSON come normalizzato. Questo è compatibile con NGSI-v2 quando non si utilizzano opzioni e restituisce i dati di contesto di un'entità individuale.  
<details><summary><strong>show/hide example</strong></summary>    
...
#### Archiviazione Persistente esempio di chiavi-valori NGSI-LD  

Ecco un esempio di PersistentStorage in formato JSON-LD come chiavi-valori. Ciò è compatibile con NGSI-LD quando si utilizza `options=keyValues` e restituisce i dati di contesto di un'entità individuale.  
<details><summary><strong>show/hide example</strong></summary>    
...
#### Archiviazione Persistente esempio normalizzato NGSI-LD  

Ecco un esempio di PersistentStorage in formato JSON-LD come normalizzato, compatibile con NGSI-LD quando non si utilizzano opzioni e restituisce i dati di contesto di un'entità individuale.  
<details><summary><strong>show/hide example</strong></summary>    
