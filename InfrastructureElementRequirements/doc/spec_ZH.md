...
实体: InfrastructureElementRequirements  
=========================================
<!-- /10-Header -->
...
[开放许可证](https://github.com/smart-data-models//dataModel.IT/blob/master/InfrastructureElementRequirements/LICENSE.md)  
...
Global description: **运行某个服务组件的基础设施元素必须具备的计算要求。**  
...
- `diskType[string]`: 枚举：'HDD'，'SSD'。基础设施元素必须具备的磁盘类型。  
- `energyEfficiencyRatio[float]`: 能效比（介于0和1之间）是基础设施元素必须具备的。  
- `gpu[boolean]`: 定义基础设施元素是否必须具有GPU。  
- `greenEnergyRatio[float]`: 必须使用为基础设施元素供电的电源的绿色能源比例（介于0和1之间）  
- `id[*]`: 实体的唯一标识符  
- `infrastructureElement[string]`: 仅当选择了特定的基础设施元素来运行组件时。  
...
- `realTimeCapable[boolean]`: 定义基础设施元素是否必须能够进行实时处理。  
- `requiredCpuUsage[float]`: 组件所需的CPU使用量。  
- `requiredMemoryUsage[number]`: 组件所需的内存使用量。  
- `seeAlso[*]`: 指向关于该项目的其他资源的URI列表  
- `source[string]`: 一个字符序列，给出实体数据的原始来源作为URL。建议为源提供者的完全限定域名，或源对象的URL。  
- `type[string]`: NGSI 实体类型。它必须是 InfrastructureElementRequirements  
<!-- /30-PropertiesList -->
...
#### 基础设施元素要求 NGSI-v2 键值示例  

这是一个以JSON格式的键值对表示的InfrastructureElementRequirements示例。当使用`options=keyValues`时，它与NGSI-v2兼容，并返回个体实体的上下文数据。  
...
#### 基础设施元素要求 NGSI-v2 标准化 示例  

这是InfrastructureElementRequirements在JSON格式的示例，已标准化。当不使用选项时，它与NGSI-v2兼容，并返回个体实体的上下文数据。  
<details><summary><strong>show/hide example</strong></summary>    
...
#### 基础设施元素要求 NGSI-LD 键值示例  

这是一个以JSON-LD格式表示的InfrastructureElementRequirements示例，以键值对的形式给出。这与NGSI-LD兼容，当使用`options=keyValues`时，返回单个实体的上下文数据。  
<details><summary><strong>show/hide example</strong></summary>    
...
#### 基础设施元素要求 NGSI-LD 标准化 示例  

这是一个基础设施元素需求的JSON-LD格式示例，已标准化。它与NGSI-LD兼容，当不使用选项时，返回单个实体的上下文数据。  
<details><summary><strong>show/hide example</strong></summary>    
