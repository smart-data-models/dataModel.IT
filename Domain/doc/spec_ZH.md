...
[开放许可证](https://github.com/smart-data-models//dataModel.IT/blob/master/Domain/LICENSE.md)  
...
Global description: **云边物联网计算连续体的域，它由一组基础设施元素组成**  
...
- `address[object]`: 邮寄地址  . Model: [https://schema.org/address](https://schema.org/address)  
	...
	- `addressRegion[string]`: 该地区的位置在于该国  . Model: [https://schema.org/addressRegion](https://schema.org/addressRegion)    
...
- `domainStatus[string]`: 枚举：'初步', '功能性', '已删除', '不受信任'。域的当前状态。  
- `id[*]`: 实体的唯一标识符  
- `isEntrypoint[boolean]`: 定义该域是否为连续体的入口域。  
- `location[*]`: GeoJSON引用该项目的内容。它可以是Point、LineString、Polygon、MultiPoint、MultiLineString或MultiPolygon  
- `name[string]`: 该物品的名称  
- `owner[array]`: 包含一个JSON编码的字符序列的列表，引用所有者（们）的唯一Id  
- `publicKey[string]`: 域的公钥（WireGuard）Base64编码密钥。  
- `publicUrl[string]`: 域的公共URL，它必须指向其暴露的API。  . Model: [https://schema.org/URL](https://schema.org/URL)  
- `seeAlso[*]`: 关于该项目的附加资源的URI列表  
- `source[string]`: 一个字符序列，给出实体数据的原始来源作为URL。建议使用源提供者的完全限定域名，或源对象的URL。  
- `type[string]`: NGSI 实体类型。它必须是域  
<!-- /30-PropertiesList -->
...
以下是JSON格式的域作为键值的示例。当使用`options=keyValues`时，它与NGSI-v2兼容，并返回个体实体的上下文数据。  
...
这是一个以JSON格式标准化的域的示例。当不使用选项时，它与NGSI-v2兼容，并返回单个实体的上下文数据。  
<details><summary><strong>show/hide example</strong></summary>    
...
这是一个以JSON-LD格式的域作为键值的示例。当使用`options=keyValues`时，它与NGSI-LD兼容，并返回个体实体的上下文数据。  
<details><summary><strong>show/hide example</strong></summary>    
...
这是一个以JSON-LD格式标准化的域的示例。当不使用选项时，它与NGSI-LD兼容，并返回个体实体的上下文数据。  
<details><summary><strong>show/hide example</strong></summary>    
