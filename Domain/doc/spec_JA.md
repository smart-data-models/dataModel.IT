...
[オープンライセンス](https://github.com/smart-data-models//dataModel.IT/blob/master/Domain/LICENSE.md)  
...
Global description: **クラウド・エッジ・IoTコンピューティング・コンティニュウムのドメインであり、これは一連のインフラストラクチャ要素で構成される。**  
...
- `address[object]`: 郵送先住所  . Model: [https://schema.org/address](https://schema.org/address)  
	...
	- `addressLocality[string]`: 住所が存在する地域およびその地域が位置する地域  . Model: [https://schema.org/addressLocality](https://schema.org/addressLocality)    
	...
	- `postOfficeBoxNumber[string]`: ポストオフィスボックス番号は、POボックスアドレスの場合。たとえば、03578  . Model: [https://schema.org/postOfficeBoxNumber](https://schema.org/postOfficeBoxNumber)    
...
- `areaServed[string]`: サービスまたは提供されたアイテムが提供される地理的な地域  . Model: [https://schema.org/Text](https://schema.org/Text)  
...
- `domainStatus[string]`: Enum:'プレリミナリ','ファンクショナル','削除済み','信頼されていない'。ドメインの現在の状態。  
- `id[*]`: エンティティの固有識別子  
- `isEntrypoint[boolean]`: このドメインが連続体のエントリーポイントドメインであるかどうかを定義します。  
- `location[*]`: アイテムへのGeojson参照。Point、LineString、Polygon、MultiPoint、MultiLineString、またはMultiPolygonのいずれかになります。  
- `name[string]`: このアイテムの名前  
- `owner[array]`: 所有者のユニークIDを参照する文字シーケンスのJSONエンコードを含むリスト  
- `publicKey[string]`: ドメインのパブリック（WireGuard）base64エンコードキー。  
- `publicUrl[string]`: ドメインの公開URLで、公開されたAPIを指す必要があります。  . Model: [https://schema.org/URL](https://schema.org/URL)  
- `seeAlso[*]`: アイテムに関する追加のリソースを指すURIのリスト  
- `source[string]`: エンティティデータの元のソースをURLとして表す文字列のシーケンス。ソースプロバイダーの完全修飾ドメイン名、またはソースオブジェクトへのURLであることを推奨する。  
- `type[string]`: NGSIエンティティタイプ。ドメインである必要があります。  
<!-- /30-PropertiesList -->
...
ここでは、キーと値のペアとしてJSON形式のドメインの例を示します。これは、`options=keyValues`を使用してNGSI-v2と互換性があり、個々のエンティティのコンテキストデータを返します。  
...
ここは、正規化されたJSON形式のドメインの例です。これは、オプションを使用しない場合、NGSI-v2と互換性があり、個々のエンティティのコンテキストデータを返します。  
<details><summary><strong>show/hide example</strong></summary>    
...
ここでは、JSON-LD形式のキー値としてのドメインの例を示します。これは、`options=keyValues`を使用してNGSI-LDと互換性があり、個々のエンティティのコンテキストデータを返します。  
<details><summary><strong>show/hide example</strong></summary>    
...
ここに、JSON-LD形式のドメインの正規化された例があります。これは、オプションを使用しない場合にNGSI-LDと互換性があり、個々のエンティティのコンテキストデータを返します。  
<details><summary><strong>show/hide example</strong></summary>    
