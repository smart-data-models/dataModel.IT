...
[Offene Lizenz](https://github.com/smart-data-models//dataModel.IT/blob/master/NetworkPort/LICENSE.md)  
...
Global description: **Netzwerkport, der von einem ServiceComponent freigegeben wird.**  
...
- `number[number]`: Nummer des Netzwerkports.  
- `owner[array]`: Eine Liste, die eine JSON-kodierte Zeichenfolge enthält, die auf die eindeutigen IDs der Eigentümer(n) verweist  
- `protocol[string]`: Protokoll des Netzwerkports. Enum: 'TCP', 'UDP', 'SCTP'  
- `seeAlso[*]`: Liste von URIs, die auf zusätzliche Ressourcen über das Element verweisen  
- `source[string]`: Eine Folge von Zeichen, die die ursprüngliche Quelle der Entitätsdaten als URL angeben. Es wird empfohlen, den vollqualifizierten Domänennamen des Quellanbieters oder die URL zum Quellobjekt zu verwenden.  
- `type[string]`: NGSI-Entitätentyp. Es muss NetworkPort sein  
<!-- /30-PropertiesList -->
...
#### Netzwerkport NGSI-v2 Schlüssel-Wert-Beispiel  

Hier ist ein Beispiel für einen NetworkPort im JSON-Format als Schlüssel-Werte. Dies ist kompatibel mit NGSI-v2, wenn `options=keyValues` verwendet wird und die Kontextdaten einer einzelnen Entität zurückgibt.  
...
#### Netzwerkport NGSI-v2 normalisiertes Beispiel  

Hier ist ein Beispiel für einen NetworkPort im JSON-Format als normalisiert. Dies ist kompatibel mit NGSI-v2, wenn keine Optionen verwendet werden und gibt die Kontextdaten einer einzelnen Entität zurück.  
<details><summary><strong>show/hide example</strong></summary>    
...
#### Netzwerkport NGSI-LD Schlüssel-Wert-Beispiel  

Hier ist ein Beispiel für einen NetworkPort im JSON-LD-Format als Schlüssel-Werte. Dies ist kompatibel mit NGSI-LD, wenn `options=keyValues` verwendet wird und die Kontextdaten einer einzelnen Entität zurückgibt.  
<details><summary><strong>show/hide example</strong></summary>    
...
#### Netzwerkport NGSI-LD normalisiertes Beispiel  

Hier ist ein Beispiel für einen NetworkPort im JSON-LD-Format als normalisiert. Dies ist kompatibel mit NGSI-LD, wenn keine Optionen verwendet werden und es die Kontextdaten einer einzelnen Entität zurückgibt.  
<details><summary><strong>show/hide example</strong></summary>    
