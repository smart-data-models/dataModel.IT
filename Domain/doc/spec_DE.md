...
[Offene Lizenz](https://github.com/smart-data-models//dataModel.IT/blob/master/Domain/LICENSE.md)  
...
Global description: **Bereich des Cloud-Edge-IoT-Rechenkontinuums, das aus einer Menge von Infrastrukturkomponenten besteht**  
...
- `address[object]`: Die Postadresse  . Model: [https://schema.org/address](https://schema.org/address)  
	...
	- `streetNr[string]`: Nummer zur Identifizierung einer bestimmten Eigenschaft auf einer öffentlichen Straße  
...
- `domainStatus[string]`: Aufzählung: 'Vorläufig', 'Funktional', 'Entfernt', 'Nicht vertrauenswürdig'. Aktueller Status der Domäne.  
- `id[*]`: Eindeutiger Identifikator der Entität  
- `isEntrypoint[boolean]`: Definiert, ob diese Domäne die Einstiegdomäne des Kontinuums ist.  
...
- `publicKey[string]`: Öffentlicher (WireGuard) base64-codierter Schlüssel der Domäne.  
- `publicUrl[string]`: Öffentliche URL der Domäne, die auf ihre freigegebene API verweisen muss.  . Model: [https://schema.org/URL](https://schema.org/URL)  
...
- `type[string]`: NGSI-Entitätstyp. Es muss Domäne sein  
<!-- /30-PropertiesList -->
...
#### Domain NGSI-v2 Schlüssel-Werte Beispiel  

Hier ist ein Beispiel für ein Domain im JSON-Format als Schlüssel-Werte. Dies ist kompatibel mit NGSI-v2, wenn `options=keyValues` verwendet wird und es die Kontextdaten einer einzelnen Entität zurückgibt.  
...
#### Bereich NGSI-v2 normalisiertes Beispiel  

Hier ist ein Beispiel für ein Domain im JSON-Format als normalisiert. Dies ist kompatibel mit NGSI-v2, wenn keine Optionen verwendet werden und es die Kontextdaten einer einzelnen Entität zurückgibt.  
<details><summary><strong>show/hide example</strong></summary>    
...
#### Domäne NGSI-LD Schlüssel-Wert-Beispiel  

Hier ist ein Beispiel für ein Domain im JSON-LD-Format als Schlüssel-Werte. Dies ist kompatibel mit NGSI-LD, wenn `options=keyValues` verwendet wird und die Kontextdaten einer einzelnen Entität zurückgibt.  
<details><summary><strong>show/hide example</strong></summary>    
...
#### Domäne NGSI-LD normalisiertes Beispiel  

Hier ist ein Beispiel für ein Domain im JSON-LD-Format als normalisiert. Dies ist kompatibel mit NGSI-LD, wenn keine Optionen verwendet werden und gibt die Kontextdaten einer einzelnen Entität zurück.  
<details><summary><strong>show/hide example</strong></summary>    
