...
[Licencia Abierta](https://github.com/smart-data-models//dataModel.IT/blob/master/PersistentStorage/LICENSE.md)  
...
Global description: **Almacenamiento persistente solicitado por un ServiceComponent.**  
...
- `accessMode[string]`: Modo del almacenamiento persistente solicitado. Enum: 'ReadWriteOnce', 'ReadOnlyMany', 'ReadWriteMany'  
...
- `hostPath[string]`: (Sólo si storageType es 'local') Ruta del host en el Elemento de Infraestructura donde se encuentra el almacenamiento persistente.  
...
- `mountPath[string]`: Ruta de montaje en el ServiceComponent del almacenamiento persistente solicitado.  
...
- `size[number]`: Tamaño del almacenamiento persistente solicitado.  
...
- `storageType[string]`: Tipo de almacenamiento solicitado, si es local se monta en el IE seleccionado del ServiceComponent, de lo contrario se monta en una infraestructura externa como un sistema de almacenamiento en la nube. Enum: 'Externo', 'Local'  
- `type[string]`: Tipo de entidad NGSI. Tiene que ser AlmacenamientoPermanente  
<!-- /30-PropertiesList -->
...
#### AlmacenamientoPermanente clave-valor NGSI-v2 Ejemplo  

Aquí hay un ejemplo de un PersistentStorage en formato JSON como clave-valor. Esto es compatible con NGSI-v2 cuando se utiliza `options=keyValues` y devuelve los datos de contexto de una entidad individual.  
...
#### AlmacenamientoPersistente NGSI-v2 normalizado Ejemplo  

Aquí hay un ejemplo de un PersistentStorage en formato JSON normalizado. Esto es compatible con NGSI-v2 cuando no se utilizan opciones y devuelve los datos de contexto de una entidad individual.  
<details><summary><strong>show/hide example</strong></summary>    
...
#### AlmacenamientoPersistente clave-valor NGSI-LD Ejemplo  

Aquí hay un ejemplo de un PersistentStorage en formato JSON-LD como clave-valor. Esto es compatible con NGSI-LD cuando se utiliza `options=keyValues` y devuelve los datos de contexto de una entidad individual.  
<details><summary><strong>show/hide example</strong></summary>    
...
#### AlmacenamientoPersistente ejemplo normalizado NGSI-LD  

Aquí hay un ejemplo de un AlmacenamientoPersistente en formato JSON-LD normalizado. Esto es compatible con NGSI-LD cuando no se utilizan opciones y devuelve los datos de contexto de una entidad individual.  
<details><summary><strong>show/hide example</strong></summary>    
