...
[Licence ouverte](https://github.com/smart-data-models//dataModel.IT/blob/master/PersistentStorage/LICENSE.md)  
...
Global description: **Stockage persistant demandé par un ServiceComponent.**  
...
- `accessMode[string]`: Mode du stockage persistant demandé. Enum:'ReadWriteOnce', 'ReadOnlyMany', 'ReadWriteMany'  
...
- `hostPath[string]`: (Only si le storageType est 'local') Chemin d'hôte dans l'élément d'infrastructure où se trouve le stockage persistant.  
...
- `mountPath[string]`: Chemin de montage dans le ServiceComponent du stockage persistant demandé.  
...
- `size[number]`: Taille du stockage persistant demandé.  
...
- `storageType[string]`: Type de stockage demandé, si local est monté dans le IE sélectionné du ServiceComponent, sinon est monté dans une infrastructure externe telle qu'un système de stockage cloud. Enum:'Externe', 'Local'  
- `type[string]`: Type d'entité NGSI. Il doit s'agir d'un PersistentStorage  
<!-- /30-PropertiesList -->
...
#### Stockage persistant clé-valeurs NGSI-v2 Exemple  

Voici un exemple de PersistentStorage au format JSON sous forme de paires clé-valeur. Ceci est compatible avec NGSI-v2 lors de l'utilisation de `options=keyValues` et renvoie les données de contexte d'une entité individuelle.  
...
#### Stockage persistant exemple normalisé NGSI-v2  

Voici un exemple de PersistentStorage au format JSON normalisé. Ceci est compatible avec NGSI-v2 lorsqu'il n'utilise pas d'options et renvoie les données de contexte d'une entité individuelle.  
<details><summary><strong>show/hide example</strong></summary>    
...
#### Stockage persistant clés-valeurs NGSI-LD Exemple  

Voici un exemple de PersistentStorage au format JSON-LD sous forme de paires clé-valeur. Ceci est compatible avec NGSI-LD lors de l'utilisation de `options=keyValues` et renvoie les données de contexte d'une entité individuelle.  
<details><summary><strong>show/hide example</strong></summary>    
...
#### Stockage persistant exemple normalisé NGSI-LD  

Voici un exemple de PersistentStorage au format JSON-LD normalisé. Ceci est compatible avec NGSI-LD lorsqu'il n'utilise pas d'options et retourne les données de contexte d'une entité individuelle.  
<details><summary><strong>show/hide example</strong></summary>    
